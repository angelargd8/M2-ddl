...
def formatRegEx(regex):
    ...
    for i in range(0, len(regex)):
        ...
        if c1 == "?": # quitar el ? si encuentra uno  
            ...
            if t[0] == ")": # si no es un valor unico, si no algo que esta en parentesis se obtiene todo lo que esta en el parentesis
                balanceador_i = 0
                balanceador_d = 1
    ...
    for i in range(0, len(regex)):
        ...
        if( i+1 < len(regex)): # verifica que se haga hasta los ultimos 2
            c2 = regex[i+1]
            res += c1
            if (c1 != "(" and c2 != ")" and c1 != "\\" and not Metachar(c2).IsOperator() and not Metachar(c1).IsBinaryOperator()):
                res += '.'
